module my_addr::addr_sui {
    ...
    // Aptos addr type.
    const ADDR_TYPE_SUI: u64 = 1;

    // Aptos addr length.
    const SUI_ADDR_LENGTH: u64 = 64;

    // Err enum.
    const ERR_INVALID_SUI_ADDR: u64 = 2002;
    ...
    public fun update_addr(addr_info: &mut AddrInfo, signature: &mut String) {
        ...
        // Check addr length.
        assert!(string::length(&addr_info::get_addr(addr_info)) == SUI_ADDR_LENGTH + 2, ERR_INVALID_SUI_ADDR);

        // Check addr type.
        assert!(addr_info::get_addr_type(addr_info) == ADDR_TYPE_SUI, addr_info::err_invalid_addr_type());
        ...
        assert!(ed25519::ed25519_verify(&sig_bytes, &pubkey_bytes, string::bytes(&addr_info_msg)), addr_info::err_signature_verify_fail());
